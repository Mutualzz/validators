...
export const gradientRegex = /^(linear|radial|conic)-gradient\([^)]*\)$/;

export const colorLikeRegex =
    /^(#(?:[A-Fa-f0-9]{3,8})|rgb\(\s*(?:\d{1,3}\s*,){2}\s*\d{1,3}\s*\)|rgba\(\s*(?:\d{1,3}\s*,){3}\s*(?:\d*\.?\d+)\s*\)|hsl\(\s*(?:\d{1,3}\s*,){2}\s*\d{1,3}%\s*\)|hsla\(\s*(?:\d{1,3}\s*,){2}\s*\d{1,3}%\s*,\s*(?:\d*\.?\d+)\s*\)|(linear|radial|conic)-gradient\([^)]*\))$/;
